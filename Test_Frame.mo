...
parameter SIunits.Position RR_lower_aft_i[3] = {RL_lower_aft_i[1], -RL_lower_aft_i[2], RL_lower_aft_i[3]} annotation(Dialog(group="Geometry"));
  ...
  Modelica.Blocks.Sources.Ramp ramp(height = 1*0.0254, duration = 0.5, startTime = 1)  annotation(
  ...
  Vehicle.Chassis.Suspension.DoubleWishboneBase RR_doubleWishboneBase1(contact_patch = RR_contact_patch, lower_aft_i = RR_lower_aft_i, lower_fore_i = RR_lower_fore_i, lower_o = RR_lower_o, tie_i = RR_tie_i, tie_o = RR_tie_o, upper_aft_i = RR_upper_aft_i, upper_fore_i = RR_upper_fore_i, upper_o = RR_upper_o) annotation(
    Placement(transformation(origin = {60, -30}, extent = {{10, -10}, {-10, 10}})));
  // FL ground interface
  Vehicle.Chassis.Suspension.Tires.BaseMF52 FL_tire(static_gamma = 0, static_alpha = 0)  annotation(
    Placement(transformation(origin = {-90, 50}, extent = {{-10, -10}, {10, 10}}, rotation = -0)));

  // FR ground interface
  Vehicle.Chassis.Suspension.Tires.BaseMF52 FR_tire(static_gamma = 0, static_alpha = 0)  annotation(
    Placement(transformation(origin = {90, 50}, extent = {{10, -10}, {-10, 10}})));
    
  // RL ground interface
  Vehicle.Chassis.Suspension.Tires.BaseMF52 RL_tire(static_gamma = 0, static_alpha = 0)  annotation(
    Placement(transformation(origin = {-90, -90}, extent = {{10, -10}, {-10, 10}}, rotation = 180)));
  
  // RR ground interface
  Vehicle.Chassis.Suspension.Tires.BaseMF52 RR_tire(static_gamma = 0, static_alpha = 0)  annotation(
    Placement(transformation(origin = {90, -90}, extent = {{-10, -10}, {10, 10}}, rotation = -180)));
    
  // Tires :D
  Modelica.Mechanics.MultiBody.Joints.Prismatic free_x(n = {1, 0, 0}, useAxisFlange = false, s(start = 0, fixed = true), v(start = 5, fixed = true)) annotation(
  ...
  Modelica.Mechanics.MultiBody.Joints.Revolute free_rot_x(n = {1, 0, 0}) annotation(
    Placement(transformation(origin = {-20, -130}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Mechanics.MultiBody.Joints.Revolute free_rot_y(n = {0, 1, 0}) annotation(
    Placement(transformation(origin = {0, -110}, extent = {{10, -10}, {-10, 10}}, rotation = 90)));
  Modelica.Mechanics.MultiBody.Joints.Revolute free_rot_z(n = {0, 0, 1}) annotation(
...
Modelica.Mechanics.MultiBody.Forces.SpringDamperParallel FL_rod(c = 62e3, s_unstretched = norm(FL_upper_o - FL_mount_node), d = 100e1)  annotation(
    Placement(transformation(origin = {-50, 70}, extent = {{-10, -10}, {10, 10}}, rotation = -0)));
Modelica.Mechanics.MultiBody.Forces.SpringDamperParallel FR_rod(c = 62e3, s_unstretched = norm(FR_upper_o - FR_mount_node), d = 100e1) annotation(
    Placement(transformation(origin = {50, 70}, extent = {{10, -10}, {-10, 10}})));
Modelica.Mechanics.MultiBody.Forces.SpringDamperParallel RL_rod(c = 62e3, s_unstretched = norm(RL_lower_o - RL_mount_node), d = 100e1) annotation(
    Placement(transformation(origin = {-34, -70}, extent = {{-10, -10}, {10, 10}}, rotation = -0)));
Modelica.Mechanics.MultiBody.Forces.SpringDamperParallel RR_rod(c = 62e3, s_unstretched = norm(RR_lower_o - RR_mount_node), d = 100e1) annotation(
...
equation
  ...
  connect(FL_tire.frame_a, ground.frame_b) annotation(
    Line(points = {{-90, 40}, {-90, 30}, {-80, 30}, {-80, 10}}, color = {95, 95, 95}));
  connect(RL_tire.frame_a, ground2.frame_b) annotation(
    Line(points = {{-90, -80}, {-90, -71}, {-80, -71}, {-80, -50}}, color = {95, 95, 95}));
  connect(RR_tire.frame_a, ground3.frame_b) annotation(
    Line(points = {{90, -80}, {90, -70}, {80, -70}, {80, -50}}, color = {95, 95, 95}));
