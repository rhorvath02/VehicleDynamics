...
model FrAxleBellcrank
  ...
  extends BobDynamics.Vehicle.Chassis.Suspension.Templates.FrAxleBase;
  ...
  BobDynamics.Vehicle.Chassis.Suspension.Linkages.Bellcrank3pu1p FL_bellcrank(pivot = FL_bellcrank_pivot, pivot_ref = FL_bellcrank_pivot_ref, pickup_1 = FL_bellcrank_pickup_1, pickup_2 = FL_bellcrank_pickup_2, pickup_3 = FL_bellcrank_pickup_3)  annotation(
  ...
  BobDynamics.Vehicle.Chassis.Suspension.Linkages.Link FL_pushrod(inboard = FL_LCA_mount, outboard = FL_bellcrank_pickup_2, link_diameter = 0.625*0.0254, joint_diameter = 0.825*0.0254)  annotation(
  ...
  Modelica.Mechanics.MultiBody.Parts.FixedTranslation FR_bellcrank_mount(r = {FL_bellcrank_pivot[1], -FL_bellcrank_pivot[2], FL_bellcrank_pivot[3]} - effective_center) annotation(
    Placement(transformation(origin = {30, 40}, extent = {{-10, -10}, {10, 10}})));
  BobDynamics.Vehicle.Chassis.Suspension.Linkages.Bellcrank3pu1p FR_bellcrank(
  ...
  Modelica.Mechanics.MultiBody.Parts.FixedTranslation FR_rod_out(r = {FL_LCA_mount[1], -FL_LCA_mount[2], FL_LCA_mount[3]} - FR_lower_o) annotation(
    Placement(transformation(origin = {110, -10}, extent = {{10, -10}, {-10, 10}}, rotation = -90)));
  BobDynamics.Vehicle.Chassis.Suspension.Linkages.Link FR_pushrod(inboard = {FL_LCA_mount[1], -FL_LCA_mount[2], FL_LCA_mount[3]}, joint_diameter = 0.825*0.0254, link_diameter = 0.625*0.0254, outboard = {FL_bellcrank_pickup_2[1], -FL_bellcrank_pickup_2[2], FL_bellcrank_pickup_2[3]}) annotation(
    Placement(transformation(origin = {90, 40}, extent = {{-10, -10}, {10, 10}}, rotation = -180)));
