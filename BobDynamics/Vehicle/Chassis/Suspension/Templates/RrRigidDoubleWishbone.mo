...
model RrRigidDoubleWishbone
  // Modelica units
  import Modelica.Math.Vectors.normalize;
  import Modelica.Math.Vectors.norm;
  import Modelica.SIunits;
  // Parameters - RL sus defn
  final parameter BobDynamics.Resources.Records.SUS.RrAxleBase RLDW;
  parameter String side = "Left" annotation(choices(choice="Left", choice="Right"));
  // Parameters
  parameter SIunits.Position upper_fore_i[3] = RLDW.upper_fore_i annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position upper_aft_i[3] = RLDW.upper_aft_i annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position lower_fore_i[3] = RLDW.lower_fore_i annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position lower_aft_i[3] = RLDW.lower_aft_i annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position upper_o[3] = RLDW.upper_outboard annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position lower_o[3] = RLDW.lower_outboard annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position tie_i[3] = RLDW.tie_inboard annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position tie_o[3] = RLDW.tie_outboard annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position wheel_center[3] = RLDW.wheel_center annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Angle static_gamma = RLDW.static_gamma annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Angle static_alpha = RLDW.static_alpha annotation(
    Dialog(group = "Geometry"));
 // Visual parameters
  ...
  Modelica.Mechanics.MultiBody.Interfaces.Frame_b midpoint_frame annotation(
    Placement(transformation(origin = {-100, 0}, extent = {{16, -16}, {-16, 16}}), iconTransformation(origin = {-100, 0}, extent = {{-16, -16}, {16, 16}})));
  // Upper wishbone
  Modelica.Mechanics.MultiBody.Joints.Revolute upper_inboard_joint(n = normalize(upper_fore_i - upper_aft_i), phi(each fixed = true, start = 0), stateSelect = StateSelect.always, animation = false) annotation(
  ...
  // Lower wisbone
  Modelica.Mechanics.MultiBody.Joints.Revolute lower_inboard_joint(n = normalize(lower_fore_i - lower_aft_i), phi(each fixed = true, start = 0), stateSelect = StateSelect.always, animation = false) annotation(
    Placement(transformation(origin = {60, -60}, extent = {{10, -10}, {-10, 10}})));
  Modelica.Mechanics.MultiBody.Parts.FixedTranslation lower_rigid_link(height = link_diameter, r = lower_o - (lower_fore_i + lower_aft_i)/2, width = link_diameter, animation = false) annotation(
    Placement(transformation(origin = {30, -60}, extent = {{10, -10}, {-10, 10}})));
  BobDynamics.Vehicle.Chassis.Suspension.Joints.xyzSphericalCompliant lower_xyzSphericalCompliant(r_rel(start = {0, 0, 0}), diameter = joint_diameter) annotation(
    Placement(transformation(origin = {10, -30}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
  // Upright
  ...
  // Tie rod
  Modelica.Mechanics.MultiBody.Joints.SphericalSpherical sphericalSpherical(rodLength = norm(tie_o - tie_i), sphereDiameter = joint_diameter, rodDiameter = link_diameter) annotation(
    Placement(transformation(origin = {40, 0}, extent = {{10, -10}, {-10, 10}})));
  
  // Generalized mass
  Modelica.Mechanics.MultiBody.Parts.Body body(r_CM = {0, 0, 0}, m = 1, animation = false, I_11 = 1, I_22 = 2, I_33 = 3, useQuaternions = false, angles_fixed = true, angles_start (each displayUnit = "rad") = {0, 0, 0}, w_0_fixed = false, w_0_start = {0, 0, 0}, enforceStates = true) annotation(
...
equation
 ...
 connect(zero_steer.y, position.s_ref) annotation(
    Line(points = {{100, -30}, {86, -30}}, color = {0, 0, 127}));
end RrRigidDoubleWishbone;
