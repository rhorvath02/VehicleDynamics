...
model RrAxleBellcrank
  ...
  extends BobDynamics.Vehicle.Chassis.Suspension.Templates.RrAxleBase;
  ...
  BobDynamics.Vehicle.Chassis.Suspension.Linkages.Bellcrank3pu1p RL_bellcrank(pivot = RL_bellcrank_pivot, pivot_ref = RL_bellcrank_pivot_ref, pickup_1 = RL_bellcrank_pickup_1, pickup_2 = RL_bellcrank_pickup_2, pickup_3 = RL_bellcrank_pickup_3)  annotation(
  ...
  BobDynamics.Vehicle.Chassis.Suspension.Linkages.Link RL_pushrod(inboard = RL_UCA_mount, outboard = RL_bellcrank_pickup_1, link_diameter = 0.625*0.0254, joint_diameter = 0.825*0.0254)  annotation(
  ...
  Modelica.Mechanics.MultiBody.Parts.FixedTranslation RR_bellcrank_mount(r = {RL_bellcrank_pivot[1], -RL_bellcrank_pivot[2], RL_bellcrank_pivot[3]} - effective_center) annotation(
    Placement(transformation(origin = {30, 40}, extent = {{-10, -10}, {10, 10}})));
  BobDynamics.Vehicle.Chassis.Suspension.Linkages.Bellcrank3pu1p RR_bellcrank(
  ...
  Modelica.Mechanics.MultiBody.Parts.FixedTranslation RR_rod_out(r = {RL_UCA_mount[1], -RL_UCA_mount[2], RL_UCA_mount[3]} - RR_upper_o) annotation(
    Placement(transformation(origin = {110, -10}, extent = {{10, -10}, {-10, 10}}, rotation = -90)));
  BobDynamics.Vehicle.Chassis.Suspension.Linkages.Link RR_pushrod(inboard = {RL_UCA_mount[1], -RL_UCA_mount[2], RL_UCA_mount[3]}, joint_diameter = 0.825*0.0254, link_diameter = 0.625*0.0254, outboard = {RL_bellcrank_pickup_1[1], -RL_bellcrank_pickup_1[2], RL_bellcrank_pickup_1[3]}) annotation(
    Placement(transformation(origin = {90, 30}, extent = {{-10, -10}, {10, 10}}, rotation = -180)));
