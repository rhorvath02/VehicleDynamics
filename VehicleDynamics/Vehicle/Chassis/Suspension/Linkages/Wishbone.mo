...
model Wishbone
  ...
  parameter SIunits.Length link_diameter "Link diameter" annotation(Dialog(group="Animation"));
  parameter SIunits.Length joint_diameter "Joint diameter" annotation(Dialog(group="Animation"));
  parameter SIunits.Position fore_i[3] "Fore inboard node coordinates" annotation(Dialog(group="Geometry"));
  parameter SIunits.Position aft_i[3] "Aft inboard node coordinates" annotation(Dialog(group="Geometry"));
  parameter SIunits.Position outboard[3] "Outboard node coordinates" annotation(Dialog(group="Geometry"));
  
  parameter SIunits.TranslationalSpringConstant[3] fore_i_c = {1e9, 1e9, 1e9} "{x, y, z}-stiffness of fore mount" annotation(Dialog(group="Mounting"));
  parameter SIunits.TranslationalSpringConstant[3] aft_i_c = {1e9, 1e9, 1e9} "{x, y, z}-stiffness of aft mount" annotation(Dialog(group="Mounting"));

  parameter SIunits.TranslationalDampingConstant[3] fore_i_d = {1e9, 1e9, 1e9} "{x, y, z}-damping of fore mount" annotation(Dialog(group="Mounting"));
  parameter SIunits.TranslationalDampingConstant[3] aft_i_d = {1e9, 1e9, 1e9} "{x, y, z}-damping of aft mount" annotation(Dialog(group="Mounting"));
