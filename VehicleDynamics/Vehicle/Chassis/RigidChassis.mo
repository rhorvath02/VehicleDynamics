...
model RigidChassis
  ...
  VehicleDynamics.Vehicle.Chassis.Suspension.Templates.FrAxleDoubleWishbone FrAxle(link_diameter = 0.025, joint_diameter = 0.030)  annotation(
    Placement(transformation(origin = {0, 47}, extent = {{-20, -20}, {20, 20}})));
  VehicleDynamics.Vehicle.Chassis.Suspension.Templates.RrAxleDoubleWishbone RrAxle(link_diameter = 0.025, joint_diameter = 0.030)  annotation(
  ...
  // Sprung mass
  Modelica.Mechanics.MultiBody.Parts.Body sprung_mass(r_CM = (RrAxle.effective_center - FrAxle.effective_center)/2, m = 200, I_11 = 30, I_22 = 40, I_33 = 50, r_0(start = {0, 0, 0.19696}, each fixed = true), enforceStates = true, useQuaternions = false) annotation(
